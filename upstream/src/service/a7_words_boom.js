...
(function() {
  getA7StorageValues(['pluginBlacklistWebsites']).then(function(result) {
    ...
    if (isUrlInBlacklist(currentUrl, blacklistPatterns)) {
      if (wordExplosionManualBlacklistOverride) {
        isInBlacklist = false;
        console.log('[WordExplosion] blacklist matched, but manual override is active');
        return;
      }
...
// 初始化前检查黑名单（与高亮黑名单同步）
function initWordExplosionWithBlacklistCheck(options = {}) {
  const manualActivation = options && options.manualActivation === true;

  if (manualActivation) {
    wordExplosionManualBlacklistOverride = true;
    isInBlacklist = false;
    console.log('[WordExplosion] manual highlight activation, allow word explosion on this page');
  }
